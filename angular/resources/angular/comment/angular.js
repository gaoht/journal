...
function $ControllerProvider() {
   ...
   */
    /**
     * 注册controller到闭包变量controller中，该闭包变量生命周期是injector的生命周期
     * @param name
     * @param constructor
     */
  ...
  this.$get = ['$injector', '$window', function($injector, $window) {
     ...
     */
      /**
       * 用于实例化一个controller
       * @param {Stirng} contructor  controller的构造函数
       * @param {Object} locals  创建该controller的时候传入的局部变量
       * 通常必须的一个local就是$scope 这样controller实例化时就将$scope上的信息初始化好了
       */
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
    ...
    function sendReq(config, reqData, reqHeaders) {
       ...
       */
         ...
         * 成功后如果开启了缓存将结果放到缓存中
         * 执行promise
         * 对$rootScope执行$apply 每个Jsonp或者Ajax都会执行$apply这个有点。。。
         * @param status 响应的status
         * @param response 响应的数据
         * @param headersString 响应的header信息
         */
...
function createHttpBackend($browser, XHR, $browserDefer, callbacks, rawDocument, locationProtocol) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials) {
    ...
    /**
     * jsonp 方法
     * 原理就是创建script标签指定src为请求的url
     * 所以jsonp返回的数据必须是可执行的javascript
     * @param url 请求的url
     * @param done 请求响应后的回调
     */
